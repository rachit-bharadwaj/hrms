    ...
    <div className="flex flex-col gap-8 w-full p-4 md:p-8 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium mt-1">
            Official holidays and corporate event schedule.
          </p>
        </div>
        <div className="flex items-center gap-4">
          <button
            onClick={() => setIsModalOpen(true)}
            className="flex items-center gap-2 px-6 py-3.5 bg-primary text-white rounded-xl text-[11px] font-black uppercase tracking-widest hover:bg-primary/80 transition-all shadow-xl shadow-slate-900/10 active:scale-95"
          >
            <Plus size={16} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-[40px] border border-slate-100 shadow-xl shadow-slate-200/20 overflow-hidden">
          <div className="grid grid-cols-7 border-b border-slate-50 bg-slate-50/20">
            {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
              <div
                key={d}
                className="py-4 text-center text-sm font-black text-slate-500 uppercase tracking-widest"
              >
                {d}
              </div>
            ))}
          ...
          <div className="grid grid-cols-7">
            {Array.from({ length: firstDayOfMonth(year, month) }).map(
              (_, i) => (
                <div
                  key={`empty-${i}`}
                  className="aspect-square border-b border-r border-slate-50 bg-slate-50/10"
                />
              ),
            )}
            {Array.from({ length: daysInMonth(year, month) }).map((_, i) => {
              ...
              return (
                <div
                  key={day}
                  className={`aspect-square border-b border-r border-slate-50 p-3 relative group hover:bg-slate-50 transition-colors ${holiday ? "bg-amber-50/20" : ""}`}
                >
                  <span
                    className={`text-sm font-bold ${holiday ? "text-amber-600" : "text-slate-500"}`}
                  >
                    {day}
                  </span>
                  {holiday && (
                    <div className="absolute inset-x-2 bottom-2 p-1.5 bg-amber-100 rounded-lg border border-amber-200 animate-in fade-in zoom-in">
                      <p className="text-[8px] font-black text-amber-700 uppercase tracking-tighter truncate">
                        {holiday.name}
                      </p>
                    </div>
                  )}
        ...
        <div className="space-y-6">
          <div className="bg-primary rounded-[40px] p-8 text-white space-y-6 shadow-2xl shadow-slate-900/10">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="p-2.5 bg-white/10 rounded-xl">
                  <Star size={18} className="text-amber-400" />
                </div>
                <h3 className="text-lg font-bold font-bricolage-grotesque">
            ...
            <div className="space-y-4">
              {holidayList.length === 0 ? (
                <p className="p-10 text-center text-xs font-bold text-slate-500 uppercase tracking-widest italic opacity-50 bg-white/5 rounded-[32px] border border-white/5">
                ...
                holidayList.map((h) => (
                  <div
                    key={h.id}
                    className="p-5 bg-white/5 rounded-3xl border border-white/5 flex items-center justify-between group hover:bg-white/10 transition-all"
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="text-sm font-bold group-hover:text-amber-400 transition-colors">
                          {h.name}
                        </h4>
                        <p className="text-sm font-bold opacity-40 uppercase tracking-widest mt-0.5">
                          {h.type}
                        </p>
                      </div>
                    </div>
