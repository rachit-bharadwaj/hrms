    ...
    <div className="flex flex-col gap-10 w-full p-4 md:p-8 pb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {isLoading ? (
            Array.from({ length: 3 }).map((_, i) => (
              <div
                key={i}
                className="h-48 w-full bg-slate-100 rounded-[32px] animate-pulse"
              />
            ))
          ) : balances.length === 0 ? (
            <div className="col-span-3 p-10 bg-slate-50 rounded-[32px] border-2 border-dashed border-slate-200 text-center text-slate-500 font-bold uppercase tracking-widest text-xs">
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <button className="text-sm font-bold text-slate-500 hover:text-slate-900 uppercase tracking-widest transition-all">
            View Full Archive
          </button>
        </div>
            ...
            <tbody className="divide-y divide-slate-50">
              {isLoading ? (
                ...
                <tr>
                  <td
                    colSpan={4}
                    className="px-10 py-24 text-center text-slate-500 font-medium"
                  >
                  ...
                  >
                    <td className="pl-10 pr-6 py-7">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-slate-500 border border-slate-100 group-hover:rotate-3 transition-transform">
                          <Calendar size={18} />
                        </div>
                    ...
                    <td className="px-6 py-7 border-l border-slate-50 bg-slate-50/5">
                      <div
                        className={`flex items-center justify-center gap-2 px-4 py-2 rounded-full font-bold text-sm uppercase tracking-widest ring-1 ${getStatusStyle(req.status)}`}
                      >
