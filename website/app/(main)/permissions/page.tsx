...
export default function PermissionsPage() {
  ...
  const getActionColor = (code: string) => {
    ...
    if (code.includes("_CREATE")) return "bg-emerald-50 text-emerald-600 ring-emerald-100";
    if (code.includes("_EDIT") || code.includes("_MANAGE") || code.includes("_UPDATE")) return "bg-amber-50 text-amber-600 ring-amber-100";
  ...
  return (
    <div className="flex flex-col gap-8 w-full pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 font-bricolage-grotesque tracking-tight">System Permissions</h1>
          <p className="text-slate-500 text-sm font-medium">Fine-grained access control identifiers for the Harbor HRMS engine.</p>
        </div>
        <button 
          onClick={() => {
          ...
          {isLoading ? (
            ...
            <div className="py-20 text-center">
              <div className="flex flex-col items-center gap-3 opacity-40">
                ...
                <p className="text-lg font-bold text-slate-400 font-bricolage-grotesque">No access policies found</p>
              </div>
            </div>
          ) : (
            Object.entries(groupedPermissions).map(([module, perms]) => (
              <div key={module} className="flex flex-col gap-4">
                <div className="flex items-center gap-3 ml-1">
                  ...
                  <h3 className="text-sm font-bold text-slate-800 uppercase tracking-widest">{module}</h3>
                  <span className="text-[10px] font-bold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-md">{perms.length}</span>
                </div>
                
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                  {perms.map((perm) => (
                    <div key={perm.id} className="group relative bg-slate-50/50 hover:bg-white border border-transparent hover:border-slate-100 hover:shadow-xl hover:shadow-blue-500/5 p-5 rounded-[24px] transition-all flex flex-col gap-3">
                      <div className="flex items-center justify-between">
                        <div className={`px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider ring-1 ${getActionColor(perm.code)}`}>
                      ...
                      <div className="flex flex-col gap-1">
                        <span className="text-xs font-mono font-bold text-slate-400 group-hover:text-slate-500 transition-colors">{perm.code}</span>
                        <p className="text-[11px] font-medium text-slate-500 leading-relaxed line-clamp-2">
                          {perm.description || "System security identifier with no specific notes."}
                        </p>
                      </div>
                    </div>
                  ))}
      ...
      {isDeleteConfirmOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-white rounded-[32px] shadow-2xl p-8 flex flex-col items-center text-center animate-in zoom-in slide-in-from-top-4 duration-300">
            ...
            <h3 className="text-xl font-bold text-slate-900 mb-2 font-bricolage-grotesque">Permanent Deletion</h3>
            <p className="text-sm text-slate-500 font-medium mb-8 px-2">
              Are you sure you want to delete <span className="text-slate-900 font-bold">{permissionToDelete?.code}</span>? This could break system access checks.
            </p>
