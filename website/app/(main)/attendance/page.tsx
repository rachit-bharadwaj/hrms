...
export default function AttendancePage() {
  ...
  const handleExport = async () => {
    ...
    window.open(`${baseUrl}/attendance/export?date=${date}&token=${token}`, "_blank");
  };
  ...
  const stats = {
    ...
    notMarked: employees.filter((e) => e.attendanceStatus === "Not Marked").length,
  };
    ...
    <div className="flex flex-col gap-8 w-full pb-20 p-4 md:p-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
         {[
           ...
           { label: "WFH", value: stats.wfh, icon: Home, color: "text-blue-600", bg: "bg-blue-50" },
           { label: "Half Day", value: stats.halfDay, icon: Clock, color: "text-amber-600", bg: "bg-amber-50" },
           { label: "Absent", value: stats.absent, icon: XCircle, color: "text-red-600", bg: "bg-red-50" },
           { label: "Missing", value: stats.notMarked, icon: AlertCircle, color: "text-slate-400", bg: "bg-slate-50" }
         ].map((stat, i) => (
           <div key={i} className="bg-white p-5 rounded-3xl border border-slate-100 flex flex-col gap-4 group hover:shadow-lg transition-all duration-300">
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="text-left text-[11px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-50 bg-slate-50/10">
                ...
                <th className="px-6 py-6 border-l border-slate-50 text-center bg-slate-50/20">Attendance Status</th>
              ...
              {isLoading ? (
                Array.from({ length: 5 }).map((_, i) => (
                  <tr key={i} className="animate-pulse">
                    ...
                    <td className="px-6 py-7"><div className="h-6 w-32 bg-slate-100 rounded-lg" /></td>
                    <td className="px-6 py-7"><div className="h-10 w-full bg-slate-100 rounded-2xl" /></td>
                    <td className="pl-6 pr-10 py-7 text-right"><div className="h-10 w-24 bg-slate-100 rounded-xl ml-auto" /></td>
                  </tr>
                ))
              ) : filteredEmployees.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-10 py-32 text-center text-slate-400 font-medium">No records found for this date.</td>
                </tr>
              ) : (
                  ...
                  <tr key={emp.id} className="group hover:bg-slate-50/50 transition-all duration-300">
                    <td className="pl-10 pr-6 py-7">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-slate-100 flex items-center justify-center text-slate-400 border border-slate-200 group-hover:rotate-3 transition-transform">
                          <User size={20} />
                        </div>
                        <div className="flex flex-col">
                          <span className="text-sm font-bold text-slate-900">{emp.firstName} {emp.lastName}</span>
                          <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{emp.employeeCode}</span>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-7 border-l border-slate-50 bg-slate-50/5 min-w-[340px]">
                      <div className="flex items-center gap-2">
                        {[
                          { label: "Present", color: "text-emerald-600 bg-emerald-50 border-emerald-100 active:bg-emerald-600 active:text-white" },
                          { label: "WFH", color: "text-blue-600 bg-blue-50 border-blue-100 active:bg-blue-600 active:text-white" },
                          { label: "Half Day", color: "text-amber-600 bg-amber-50 border-amber-100 active:bg-amber-600 active:text-white" },
                          { label: "Absent", color: "text-red-600 bg-red-50 border-red-100 active:bg-red-600 active:text-white" }
                        ].map((status) => (
                          <button
                            key={status.label}
                            onClick={() => handleStatusChange(emp.id, status.label)}
                            className={`flex-1 py-3.5 px-3 rounded-[18px] text-[10px] font-bold uppercase tracking-wider border-2 transition-all active:scale-95 ${
                    ...
                    <td className="pl-6 pr-10 py-7 text-right">
                       <Link 
                         ...
                         className="text-[10px] font-bold text-blue-600 bg-blue-50 px-4 py-2 rounded-full hover:bg-blue-600 hover:text-white transition-all uppercase tracking-widest"
                       >
                         History
                       </Link>
                    </td>
