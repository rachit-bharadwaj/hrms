      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium">
            Define and manage organization access levels and their associated
            permissions.
          </p>
        </div>
        <button
          ...
          className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 py-3 rounded-xl text-sm font-bold transition-all shadow-xl shadow-blue-500/20 active:scale-95 whitespace-nowrap"
        >
          <Plus size={18} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-6">
        {isLoading ? (
          Array.from({ length: 3 }).map((_, i) => (
            <div
              key={i}
              className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm animate-pulse"
            >
          ...
          filteredRoles.map((role) => (
            <div
              key={role.id}
              className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-blue-500/5 transition-all group flex flex-col items-start text-left relative overflow-hidden"
            >
              <div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                <button
                  ...
                  className="p-2 hover:bg-blue-50 rounded-xl text-slate-500 hover:text-blue-600 transition-all border border-transparent hover:border-blue-100"
                >
                  <Edit2 size={16} />
                </button>
                <button
                  ...
                  className="p-2 hover:bg-red-50 rounded-xl text-slate-500 hover:text-red-600 transition-all border border-transparent hover:border-red-100"
                >
                  <Trash2 size={16} />
                </button>
              </div>

              <div className="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center text-blue-600 mb-5 shadow-inner ring-1 ring-blue-100">
                <Shield size={24} />
              </div>
      ...
      {isDeleteConfirmOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-sm bg-white rounded-[32px] shadow-2xl p-8 flex flex-col items-center text-center animate-in zoom-in slide-in-from-top-4 duration-300">
            ...
            {deleteError && (
              <div className="bg-red-50 text-red-600 p-4 rounded-xl text-[11px] font-bold border border-red-100 flex items-start gap-3 mb-6 w-full text-left">
            ...
            <div className="flex flex-col w-full gap-3">
              <button
                ...
                disabled={isDeleting}
                className="w-full bg-red-600 hover:bg-red-700 disabled:opacity-70 text-white py-4 rounded-xl text-sm font-bold transition-all shadow-xl shadow-red-500/20 active:scale-95 flex items-center justify-center gap-2"
              >
                {isDeleting ? (
              ...
              <button
                ...
                disabled={isDeleting}
                className="w-full bg-slate-50 hover:bg-slate-100 disabled:opacity-50 text-slate-600 py-4 rounded-xl text-sm font-bold transition-all"
              >
                Cancel
              </button>
            </div>
